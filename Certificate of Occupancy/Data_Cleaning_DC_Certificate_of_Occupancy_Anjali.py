...
df_count = df[['CATEGORY']].groupby(['CATEGORY'])['CATEGORY'] .count() .reset_index(name='count') .sort_values(['count'], ascending=False)

df_count_gt_1000 = df_count[df_count['count']>1000]
ax = sns.countplot(x="CATEGORY", data=df)
plt.xlabel('category', horizontalalignment='right', x=1.0)
plt.xticks(rotation=90)
plt.title('Distribution of all the categories')
plt.tight_layout()
plt.show()

df_energy = pd.read_csv('..\Data\Building_Energy_Benchmarking.csv')
